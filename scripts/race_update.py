...
def write_upcoming_stub(year: int, upcoming: pd.Series) -> None:
    """
    Write a minimal upcoming_prediction.json with the next race's name and
    date but no predictions yet. Used when the race isn't in the dataset
    yet (no qualifying / entry list). Lets update_readme.py show the right
    race name + 'prediction will appear after qualifying' instead of
    leaving the previous race's now-stale prediction in place.
    """
        ...
        json.dumps(
            {
                ...
                "race_name": str(upcoming["EventName"]),
                "race_date_utc": str(upcoming["RaceEndUtc"]),
                "predictions": [],
                "generated_at": dt.datetime.now(dt.timezone.utc).isoformat(),
                "note": "Stub written before qualifying / entry list available. "
                        "Full prediction will be regenerated by the Saturday cron.",
            },
            indent=2,
            default=str,
        )
    )
    print(f"[upcoming] Wrote stub for {upcoming['EventName']}")
...
def refresh_upcoming_prediction(featured: pd.DataFrame, year: int, upcoming: pd.Series) -> bool:
    ...
    Returns True if a prediction was written. If the race isn't yet in the
    dataset, write a stub so the README shows the right race name.
    """
    ...
    if race_mask.sum() == 0:
        ...
        write_upcoming_stub(year, upcoming)
        return False

...
with mlflow.start_run(run_name=build_run_name("backtest", "historical", "2020-2026")) as run:
    set_run_context(
        component="backtest",
        script="backtest.py",
        stage="evaluation",
        dataset="data/processed/all_races.parquet",
        split="rolling-train-prior-years-test-next-year",
        notes="Historical backtest across seasons",
    )
    log_params({
...
# Regulation change vs normal
    ...
    mlflow.log_metric("overall_spearman", float(overall_sp))
    mlflow.log_metric("overall_winner_pct", float(overall_win))
    mlflow.log_metric("overall_top3_overlap", float(overall_t3))
    mlflow.log_metric("overall_top5_overlap", float(overall_t5))
    mlflow.log_metric("overall_avg_error", float(overall_err))

    log_dataframe_artifact(all_df, "tables", "per_race_backtest_results.csv")
    log_dataframe_artifact(ss, "tables", "season_backtest_summary.csv")
    log_json_artifact(
        {
            "run_id": run.info.run_id,
            "overall_spearman": float(overall_sp),
            "overall_winner_pct": float(overall_win),
            "seasons": len(ss),
            "races": len(all_df),
        },
        "reports",
